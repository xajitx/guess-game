...
def load_initial_questions():
    ...
    initial_questions = []
    for i in range(4):
        initial_questions.append(model.get_question_by_id(i+1)) # is character real
    '''we have to add 4 intial questions which will classify the whole data so that if all three are result to unsure '''
...
def get_nearby_objects_values(objects_values, how_many=10):
    ...
    if sorted_objects_values:
        nearby_objects_values = [(sorted_objects_values[i][0], model.get_object_by_id(sorted_objects_values[i][1])) for
                                 i in range(how_many)]
    else:
...
def entropy(objects, question):
    ...
    positives = model.get_num_positives(objects, question[0]) * 1.0
    negatives = model.get_num_negatives(objects, question[0]) * 1.0
    total = len(objects)
    ...
    if entropy != 0:
        entropy = 1 / entropy  # minimizes rather than maximizes
    else:
        entropy = float('inf')
...
def simple_entropy(objects, question):
    ...
    objects = tuple(objects)  # necessary for SQL IN statement to work
    positives = model.get_num_positives(objects, question[0])
    negatives = model.get_num_negatives(objects, question[0])
    unknowns = model.get_num_unknowns(objects, question[0])
...
def choose_question(initial_questions, objects_values, asked_questions, how_many=15):
    ...
    if initial_questions:
        ...
        for question in questions:  # loop through all the questions
            if not (str(question[0]) in asked_questions.keys()):  # if we have not already asked it, condider it
    ...
    return question,initial_questions
...
def update_local_knowledgebase(objects_values, asked_questions, question_id, answer):
    ...
    if not (answer in [yes, no, unsure]):
        ...
        for weight in weights:
            if str(weight[0]) in objects_values.keys():

                if weight[2] > WEIGHT_CUTOFF:
                    value = WEIGHT_CUTOFF
                elif weight[2] < -1 * WEIGHT_CUTOFF:
                    value = -1 * WEIGHT_CUTOFF
                elif weight[2] < unsure:
                    value = weight[2] / 2  # lessens impact of strong negatives
                else:
                    value = weight[2]
                ...
                objects_values[str(weight[0])] += answer * value
...
def learn_character(asked_questions, name):
    ...
    if name.strip() != '':
        ...
        if object:  # character in database
            learn(asked_questions, object[0])
            return object[0]
        else:
...
#getting error in setting the dictionay
...
if __name__ == '__main__':
    ...
    for object in objects:
        temp_obj+=(object[0],)
   # objects = [object[0][0] for object in objects]
    objects = tuple(temp_obj)
    questions = model.get_questions()
